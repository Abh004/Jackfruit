...
Environment variables (set by docker-compose):
  REPLICA_ID   : unique integer ID
  PORT         : port this server listens on
  PEERS        : comma-separated peer URLsx
"""
...
async def request_vote(req: VoteRequest):
    ...
    if already_voted or not log_ok:
        log.info(
            f"Rejecting vote for {req.candidate_id}: already_voted={already_voted} log_ok={log_ok}"
        )
        return VoteResponse(term=state.current_term, vote_granted=False)
...
async def append_entries(req: AppendEntriesRequest):
    ...
    if req.prev_log_index >= 0:
        if req.prev_log_index > state.last_log_index():
            ...
            log.warning(
                f"Missing entries: our log ends at {state.last_log_index()}, leader wants prev={req.prev_log_index}"
            )
            return AppendEntriesResponse(
                term=state.current_term,
                success=False,
...
async def sync_log(req: SyncLogRequest):
    ...
    log.info(
        f"Sync-log requested from index {req.from_index}: sending {len(missing)} entries"
    )
    ...
    for peer, result in zip(PEER_URLS, results):
        ...
        if result.status_code == 200:
            ...
            if data.get("success"):
                ...
                asyncio.create_task(
                    _catchup_peer(peer, data.get("match_index", -1) + 1)
                )
...
async def _catchup_peer(peer_url: str, from_index: int):
    ...
    try:
        ...
        log.info(
            f"Catch-up sent {len(missing)} entries to {peer_url} from index {from_index}"
        )
...
async def _start_election():
    ...
    async with httpx.AsyncClient(timeout=0.5) as client:
        tasks = [
            client.post(f"{peer}/request-vote", json=vote_request) for peer in PEER_URLS
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
